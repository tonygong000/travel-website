...
- 前端：点击右上角或底部的「开始记录」按钮新增；若要修改已有游记，可在「故事卡」里点「上传/编辑游记」按钮，此时只会弹出“游记内容”单栏，保存后仅更新对应旅程的游记文本；若要修改整条旅程数据，可在「时空坐标」时间轴卡片中点击「修改旅程数据」，弹窗会填充现有字段并允许更新地图/时间轴信息后写回数据库。地图区域现已锁定中国视图，支持鼠标拖拽、滚轮缩放、「＋/－」按钮缩放、自动缩放以适配所有足迹、按时间顺序绘制路线动画、带日期的标记气泡与聚合气泡，点击城市标记会自动放大并有“重置到中国”按钮快速回到全国全览。
...
## 主要功能
- **Where & When**：基于经纬度的地图描点 + 时间轴（中国视图专用）。
  - 鼠标拖拽、滚轮/按钮缩放、城市点击聚焦，并提供“重置到中国”按钮回到全国全览。
