...
4. 新增或更新旅程（可选）：
   - 前端：点击右上角或底部的「开始记录」按钮新增；若要修改已有游记，可在「故事卡」里点「上传/编辑游记」按钮，此时只会弹出“游记内容”单栏，保存后仅更新对应旅程的游记文本；若要修改整条旅程数据，可在「时空坐标」时间轴卡片中点击「修改旅程数据」，弹窗会填充现有字段并允许更新地图/时间轴信息后写回数据库。地图区域新增「＋/－」缩放按钮与标记点击聚焦，便于放大美国等区域查看精确城市位置。
